...
//  rail hold two selections instead of one; nothing else has to move.
// ═══════════════════════════════════════════════════════════════════════════

import { useCallback, useEffect, useLayoutEffect, useMemo, useRef, useState } from "react";
import { useReactToPrint } from "react-to-print";
import { Calendar, ChevronLeft, ChevronRight, FileText, Pill, Printer, RefreshCw, X } from "lucide-react";
                ...
                >
                    ...
                    <div className="rxv-body">
                        ...
                        <nav className="rxv-rail" aria-label="Past visits">
                            <p className="rxv-rail-label">{visits.length} visit{visits.length === 1 ? "" : "s"}</p>
                            <ul className="rxv-rail-list">
                                {visits.map((v, i) => {
                                    const isActive = v.id === visit.id;
                                    return (
                                        <li key={v.id}>
                                            <button
                                                type="button"
                                                className={`rxv-rail-item${isActive ? " is-active" : ""}`}
