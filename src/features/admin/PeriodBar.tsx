...
export function PeriodBar({
    period, range, onChange, onRefresh, busy, children, scopeSlot,
}: {
    period: PeriodState;
    ...
    children?: ReactNode;
    /** Scope filter slot (e.g., doctor performance filter for admin doctors). */
    scopeSlot?: ReactNode;
}) {
    const today = clinicToday();

    return (
        <div className="flex flex-wrap items-center gap-[8px] rounded-[var(--cs-radius)] border border-[var(--cs-line)] bg-[var(--cs-card)] px-[12px] py-0 shadow-[var(--cs-shadow)] h-[46px] min-h-[46px]">
            {/* Period section */}
            <div className="flex items-center gap-[6px] h-[28px]">
                <span className="flex items-center gap-[4px] text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--cs-label)] whitespace-nowrap h-[28px]">
                    <CalendarDays size={13} className="text-[var(--cs-blue)]" /> Period:
                </span>
                ...
                <div className="flex items-center gap-[4px] h-[28px]">
                    {PRESETS.map((p) => (
                        <button
                            ...
                            className={
                                "h-[28px] cursor-pointer rounded-full border px-[10px] py-0 text-[11px] font-semibold transition-colors outline-none flex items-center justify-center " +
            ...
            {/* Visual Divider between Period section and Scope section */}
            {scopeSlot && (
                <div className="h-[20px] w-[1px] bg-[var(--cs-line-strong)] mx-[4px] hidden sm:block flex-none" />
            )}

            {scopeSlot}

            <div className="ml-auto flex items-center gap-[5px] flex-nowrap">
                {children}
                ...
                <input
                    ...
                    className="h-[28px]! rounded-[6px]! border! border-[var(--cs-line-strong)]! bg-[var(--cs-page)]! px-[6px]! text-[11px]! text-[var(--cs-ink)] outline-none focus:border-[var(--cs-blue)]!"
                />
                <span className="text-[10.5px] text-[var(--cs-faint)]">to</span>
                <input
                    type="date"
                    aria-label="To date"
                ...
                <button
                    ...
                    aria-label="Refresh"
                    title="Refresh"
                    className="grid h-[28px] w-[28px] cursor-pointer place-items-center rounded-[6px] border border-[var(--cs-line-strong)] bg-transparent text-[var(--cs-faint)] outline-none transition-colors hover:bg-[var(--cs-blue-soft)] hover:text-[var(--cs-blue)]"
                >
