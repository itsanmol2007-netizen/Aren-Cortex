...
/**
 * The value that fills the prescription template's "View Your Prescription"
 * dynamic-URL button — `prescriptions.share_token`, a random per-prescription
 * string generated by a DB trigger (migration 20260909_prescription_share_token).
 *
 * The BSP/Meta stores the button's BASE url on the approved template
 * (`https://www.arenode.com/prescriptions/{{1}}`); we only ever send `{{1}}`,
 * which is this token. The public page at that URL resolves the token through
 * the `rx-preview` edge function. If a prescription somehow has no token
 * (older row the backfill missed), we send the message WITHOUT the button
 * rather than fail it — "your prescription is ready" still lands.
 */
async function prescriptionShareToken(sb, prescriptionId) {
    ...
    const { data, error } = await sb
        ...
        .maybeSingle();
    if (error) {
        console.warn(`[messaging] could not read share_token for ${prescriptionId}: ${error.message}`);
        return null;
    }
...
export async function sendMessage(input) {
    ...
    // The prescription template's dynamic button parameter. An explicit
    // documentUrl from the caller wins (a future "here is the PDF" flow);
    // otherwise it's the prescription's own share token.
    const buttonParam =
        input.purpose === "prescription"
            ? input.documentUrl ?? (await prescriptionShareToken(sb, input.prescriptionId))
            : null;
    try {
