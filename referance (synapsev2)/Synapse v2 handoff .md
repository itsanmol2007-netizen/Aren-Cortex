...
> **Environment note (2026-08-08, explicit instruction from Anmol — do not "fix" this).** `.env` is committed to the main Cortex repo, deliberately, and is no longer gitignored there. It holds only `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` for `arenod` (`ieimvjprtltancxapuzg`) — the anon key is meant to be public (it ships in every built browser bundle regardless; `VITE_`-prefixed vars are never actually secret) and RLS is the real security boundary, not keeping that key hidden. No service-role key or DB password belongs there, ever. The repo is private, single-owner, used across Anmol's own devices, and hand-retyping the key on every clone was costing real time to character-level typos. If this project ever needs a service-role key for tooling, it does **not** go in that committed `.env` — treat that one file, and only that file, as the exception.
