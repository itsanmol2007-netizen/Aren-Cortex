...
| `doctors.email` | **added 2026-09-01** — a real, optional, doctor-owned contact address. Distinct from, and must never be confused with, the Supabase Auth email on `auth.users` (`{digits}@aren.internal`, derived from the phone number purely so phone-based login has something email-shaped to authenticate against). That synthetic address is a login artifact: showing it publishes the doctor's phone number, and letting anyone "change" it rewrites the identity `phoneToAuthEmail()` derives and locks the account out. `doctors.email` is the only address any UI may display or edit; NULL renders as "no email on file", never as a fallback to the auth address. Settings made exactly this mistake and was corrected this round — see `cortex-standing-rules.md`. |
| `user_devices` | **added 2026-09-01** — one row per (account, browser install), so "which machines am I signed in on" has a real answer instead of Settings saying "not tracked yet". `device_key` is a random UUID minted client-side into localStorage — identifies the INSTALL, not the person. **No IP address and no location are stored, by design.** RLS is per-USER (`user_id = auth.uid()`), not per-hospital — a colleague at the same clinic must not enumerate or revoke someone else's devices. `revoked_at` is enforced by the client that owns the row on its next boot (`touchThisDevice`, `src/lib/db/devices.ts`), which is a real but not-instant remote sign-out; `supabase.auth.signOut({ scope: 'global' })` is the immediate server-side kill switch and Settings' UI says which is which. |
| `plans` / `plan_entitlements` / `subscriptions` / `subscription_requests` | **schema added 2026-08-29, given live content 2026-09-01** — Plan → Subscription → Clinic. `plans.name` is renameable and HAS been renamed (`multi` went from "AREN Nova" to "AREN Constellation", 2026-09-11); nothing may branch on it, `plans.code` is the only stable key. `plan_entitlements.limit_value = NULL` means unlimited, never zero. `subscriptions` is read-only even for the owning clinic's `admin`/`owner` role — no role can assign or extend its own plan through the anon key, by design (see `ADMIN-PANEL-INTEGRATION.md` §4a). `subscription_requests` is the doctor-facing ask ("upgrade", "add seats", "invoice", …); a clinic can file and read its own, nobody can resolve one — the un-built other half is an admin approval queue, same pattern as `composition_requests`. |
